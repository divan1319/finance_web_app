...
class Entradas
{
    ...
    /**
     * Registra una nueva entrada y guarda la imagen de factura en el servidor.
     *
    ...
    /**
     * Elimina una entrada por su ID y borra su factura del servidor.
     *
     * @param  int  $id     ID del registro
     * @param  int  $userId ID del usuario
    ...
    {
        $entrada = $this->buscarPorId($id, $userId);
        ...
        if ($stmt->rowCount() > 0) {
            $this->gestorFacturas->eliminar($entrada['factura_url']);
            return true;
        }
